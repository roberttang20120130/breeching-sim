...
TILE = 32
MAP_SIZE = 20
WIDTH = HEIGHT = TILE * MAP_SIZE
FOV_ANGLE = math.radians(130)
MAX_VIEW_DIST = 12 * TILE

def load_thmap_file():
    # Open a file dialog to pick a .thmap
    root = tk.Tk()
...
MAP_SIZE = len(GAME_MAP)  # automatically detect map size
WIDTH = HEIGHT = TILE * MAP_SIZE
...
class Actor:
    def __init__(self,x,y,color):
        ...
        self.cooldown=0
    def rect(self): r=self.hitbox.copy(); r.center=(self.x,self.y); return r

player=Actor(2*TILE,2*TILE,(0,255,0))
enemies=[]
for y,row in enumerate(GAME_MAP):
    for x,t in enumerate(row):
        if t==2: enemies.append(Actor(x*TILE+TILE//2,y*TILE+TILE//2,(255,0,0)))

bullets=[]

def walkable(x,y):
    gx=int(x//TILE); gy=int(y//TILE)
    return 0<=gx<MAP_SIZE and 0<=gy<MAP_SIZE and GAME_MAP[gy][gx]!=1

def astar(start,goal):
    sx,sy=start; gx,gy=goal
    open=[(0,(sx,sy))]; came={}; g={ (sx,sy):0 }
    while open:
...
def cast_fov(surface, origin, angle):
    ...
    for i in range(steps+1):
        ...
        for d in range(0, MAX_VIEW_DIST, 2):
            ...
            y = oy + math.sin(a)*d
            gx = int(x//TILE); gy = int(y//TILE)
            if not (0 <= gx < MAP_SIZE and 0 <= gy < MAP_SIZE): break
            if GAME_MAP[gy][gx] == 1:
                points.append((x,y))
                break
        else:
...
while running:
    ...
    keys=pygame.key.get_pressed(); speed=140*dt; move_x=move_y=0
    ...
    for en in enemies:
        gx=int(en.x//TILE); gy=int(en.y//TILE)
        px=int(player.x//TILE); py=int(player.y//TILE)
        path=astar((gx,gy),(px,py))
        if path:
            tx,ty=path[0]
            vx=(tx*TILE+TILE/2-en.x); vy=(ty*TILE+TILE/2-en.y)
    ...
    for b in bullets[:]:
        b[0]+=math.cos(b[2])*400*dt; b[1]+=math.sin(b[2])*400*dt
        ...
        if b[3]==0:
            ...
            if rect.colliderect(player.rect()): bullets.remove(b)
